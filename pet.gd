...
var form: Form = Form.CAT    # cat walks and jumps; parrot flies instead
...
func _ready() -> void:
	...
	_helper_path = ProjectSettings.globalize_path("res://helpers/window_list")
	...
	if OS.get_environment("PET_PLAY") != "":
		_toggle_play()
	# PET_FORM=parrot starts as a parrot (handy for testing).
	if OS.get_environment("PET_FORM").to_lower() == "parrot":
		form = Form.PARROT
	if form == Form.PARROT:
	...
	if FileAccess.file_exists(_helper_path):
		...
		push_warning("helpers/window_list not found — pet will only walk on the Dock line. "
			+ "Build it with the clang command at the top of helpers/window_list.c")
...
func _init_bridge() -> void:
	...
	if bridge_dir == "":
		bridge_dir = OS.get_environment("HOME").path_join(".desktop-pet")
...
func _rebuild_platforms() -> void:
	...
	_mutex.unlock()
	# The helper and Finder report macOS points; Godot's DisplayServer uses
